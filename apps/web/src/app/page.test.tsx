...
describe("HomePage", () => {
  it("renders the empty initial analysis screen", () => {
    const markup = renderToStaticMarkup(createElement(HomePage));

    expect(markup).toContain("기본 분석");
    expect(markup).toContain("nado-empty-result");
    expect(markup).toContain("아직 분석 결과가 없어요");
    expect(markup).toContain(
      "영어 문장이나 짧은 문단을 입력하고 분석을 실행하면",
    );
  ...
  it("defines the empty result state styles", () => {
    expect(styles).toContain(".nado-empty-result");
    expect(styles).toContain("min-height: min(420px, calc(100dvh - 220px))");
  });
});
