...
| 역할                       | 파일                                                                       |
| -------------------------- | -------------------------------------------------------------------------- |
| 정적 검증·E2E              | [CI workflow](../../.github/workflows/ci.yml)                              |
...
CI는 PR 변경과 `main` push에서 lint, typecheck, test, build와 E2E를 실행한다. 같은 검증을 로컬에서 실행하는 방법은 [로컬 개발 안내](local-development.md)를 참고한다.
